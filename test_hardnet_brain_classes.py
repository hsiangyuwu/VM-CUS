...
def main():
    ...
    val_img_paths = glob('C:\\Users\\user\\brain_hardnet_only\\Data\\validation_classes\\img_train\\*')
    val_mask_paths = glob('C:\\Users\\user\\brain_hardnet_only\\Data\\validation_classes\\lab_val\\') 
        ...
        with torch.no_grad():
            ...
            for i, (input, img_path) in tqdm(enumerate(val_loader), total=len(val_loader)):
                ...
                for ij in range(0, 1):
                    ...
                    # grond truth
                    temp_file_name2 = val_mask_paths[0] + temp_file_name
                    maskMat = scipy.io.loadmat(temp_file_name2)
                    ...
                    mask_tmp = mask_tmp.transpose((2, 0, 1, 3))
                    out_mask, out_maxvals_mask = get_max_preds(mask_tmp)                    
                    out_mask = out_mask.squeeze()
                    out_maxvals_mask = out_maxvals_mask.squeeze()
                    
                    log = []
                    log = pd.DataFrame(index=[], columns=[
                        'Name', 'xpa', 'ypa', 'mvalue', 'class'
                    ])
                    
                    class_gt =re.search('^\d*(?=_)', temp_file_name).group()
                    class_gt = str(int(class_gt) + 1)
                    ...
                    log['class'][0] = class_gt
                    log.to_csv('Fold'+str(Fold)+'_output/%s_GoundTruth_csv/'%val_args.name+temp_file_name[0:-4] + '.csv', index=False)            
